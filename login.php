                ...
                <div class="card-body">
                    <form id="login-form">
                        ...
                        <div class="form-group">
                            <label for="username" class="control-label">Admin</label>
...
<script>
    ...
    $('#autologin_check').change(function() {
        if($(this).is(':checked')) {
            ...
            $('#username').val('quickuser'); 
            $('#password').val('quick123');
            
            // Remove existing quick_login hidden fields to avoid duplicates
            $('input[name="quick_login"]').remove();
            
            // Add a hidden field to identify quick login
            $('#login-form').append('<input type="hidden" name="quick_login" value="1">');
            
            // Trigger form submission automatically
            $('#login-form').submit();
        } else {
            // When unchecked, remove the hidden field
            $('input[name="quick_login"]').remove();
        }
    });
        ...
        $.ajax({
            ...
            success: function(resp){
                if(resp == 1){
                    ...
                    // Remove quick_login hidden field on failure
                    $('input[name="quick_login"]').remove();
                }
            }
        })
